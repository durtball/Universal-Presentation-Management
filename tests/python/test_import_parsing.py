...
def test_session_title_alone_is_not_used_as_composite_identity() -> None:
    first = _wide_row_session(
        _normalized({"Session Title": "Repeated title", "Presentation ID": "P-1"})
    )
    second = _wide_row_session(
        _normalized({"Session Title": "Repeated title", "Presentation ID": "P-2"})
    )

    assert "session_code" not in first
    assert "session_code" not in second
