...
- **Authentication/RBAC and SMB:** Site production Compose now requires Site-local authentication. Opaque expiring sessions, CSRF, lockout, role/granular backend permission checks, logout, security logs, and a bootstrap Site-local administrator operate entirely from Site PostgreSQL. Existing event deployment snapshots include only Central users scoped to the destination Site and deploy scrypt verification material—not plaintext or SMB passwords. Snapshot application upserts Central-managed identities by UUID, safely disables username collisions, preserves all Site-local fields, and queues local Samba revocation when authorization is removed. Central and Site now run independent durable, periodic managed-Presentations reconciliation against deployment-local canonical data and Media Storage. The read-only share exposes current confirmed versions through schedule, room, presenter, and all-presentation views; it removes stale paths and uses atomic hard links or a verified controlled cross-filesystem cache without exposing storage keys. Permission tuning, account recovery, SMB-user activity correlation, and Windows end-to-end verification remain incomplete.
