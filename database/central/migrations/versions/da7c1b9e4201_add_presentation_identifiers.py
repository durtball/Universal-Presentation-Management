"""add stable operator-facing presentation identifiers

Revision ID: da7c1b9e4201
Revises: f6a21c84d903
"""
...
revision: str = "da7c1b9e4201"
down_revision: str | Sequence[str] | None = "f6a21c84d903"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        presentation_identifier = COALESCE(NULLIF(BTRIM(presentation_code), ''),
          'UPM-CENTRAL-' || UPPER(RIGHT(REPLACE(presentation_id::text, '-', ''), 12))),
    ...
    op.create_index(
        "ix_central_presentations_external_identifier",
        "presentations",
...
def downgrade() -> None:
    op.drop_index("ix_central_presentations_external_identifier", table_name="presentations")
    op.drop_index("uq_central_presentations_event_identifier", table_name="presentations")
