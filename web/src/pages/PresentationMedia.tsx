...
export function PresentationMedia({ mode }: { mode: Mode }) {
  ...
  const data = useApi(async (signal) => {
    ...
    const suggestedIds = [...new Set(workspace.imports.flatMap((item) => item.match_candidates.slice(0, 1).map((candidate) => candidate.presentation_id)))].slice(0, 50);
    const candidateResponse = await api.mediaCandidates(activeEvent, "", signal, suggestedIds);
...
export function MatchControl({ item, candidates, eventId, onDone, csrf }: { item: CentralMediaImport; candidates: PresentationMatchCandidate[]; eventId: string; onDone: () => void; csrf: string | null }) {
  ...
  const [resolvedSuggestion, setResolvedSuggestion] = useState<PresentationMatchCandidate | undefined>(
    candidates.find((candidate) => candidate.presentation_id === selectedCandidate(item)),
  );
  useEffect(() => {
