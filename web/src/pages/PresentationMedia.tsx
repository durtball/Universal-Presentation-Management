...
export function PresentationMedia({ mode }: { mode: Mode }) {
  ...
  return <Page eyebrow={mode === "central" ? "Global media control" : "Site-local media"} title="Presentation Media" description={mode === "central" ? "Upload, match, version, and track presentation media across Sites." : "Local uploads remain fully available while Central or the WAN is unavailable."} actions={<button className="button button--primary" disabled={!activeEvent} onClick={() => setUpload(true)}>Upload media</button>}>
    ...
    {data.loading ? <Loading /> : data.error ? <ErrorSurface error={data.error} onRetry={data.refresh} /> : data.data ? <>
      ...
      {mode === "central" ? <CentralReviewQueue initialItems={data.data.unmatched as CentralMediaImport[]} eventId={activeEvent} csrf={session.csrfToken} /> : null}
...
export function CentralReviewQueue({ initialItems, eventId, csrf }: { initialItems: CentralMediaImport[]; eventId: string; csrf: string | null }) {
  ...
  const [sort, setSort] = useState<"received" | "filename">("received");
  ...
  const visible = useMemo(() => items.filter((item) => reviewFilter(item, filter) && reviewVisible(item, search)).sort((left, right) => sort === "filename" ? left.original_filename.localeCompare(right.original_filename) : right.created_at.localeCompare(left.created_at)), [filter, items, search, sort]);
  const pageRows = visible.slice(page * REVIEW_PAGE_SIZE, (page + 1) * REVIEW_PAGE_SIZE);
  const markConfirmed = (ids: string[]) => { setItems((current) => current.map((item) => ids.includes(item.media_import_id) ? { ...item, match_state: "confirmed", import_state: "assigned" } : item)); setChecked((current) => new Set([...current].filter((id) => !ids.includes(id)))); };
  const confirmOne = async (item: CentralMediaImport) => {
    ...
    try { await centralApi(csrf).assignMedia(item.media_import_id, presentationId); markConfirmed([item.media_import_id]); }
  ...
  return <Panel title="Presentation Media work queue" description="Compact operator review. Suggestions remain unconfirmed until an operator acts."><div className="media-toolbar media-toolbar--review"><label>Search<input className="input" value={search} onChange={(event) => { setSearch(event.target.value); setPage(0); }} placeholder="Filename, presenter, session, or ID" /></label><label>Filter<select className="input" value={filter} onChange={(event) => { setFilter(event.target.value); setPage(0); }}><option value="needs_review">Needs Review</option><option value="suggested">Suggested Match</option><option value="unmatched">No Match</option><option value="confirmed">Confirmed</option><option value="errors">Errors</option><option value="all">All</option></select></label><label>Sort<select className="input" value={sort} onChange={(event) => setSort(event.target.value as "received" | "filename")}><option value="received">Newest received</option><option value="filename">Filename</option></select></label><button className="button" onClick={() => setChecked(new Set(visible.filter(isGood).map((item) => item.media_import_id)))}>Select All Good Matches</button><button className="button button--primary" disabled={!checked.size} onClick={() => void confirmSelected()}>Confirm Selected ({checked.size})</button><button className="button" disabled={Boolean(rescan && !rescan.finished)} onClick={() => void centralApi(csrf).rescanUnmatchedMedia(eventId).then((progress) => { setItems((current) => mergeMediaRows(current, progress.items)); setRescan(progress); })}>Rescan All Unmatched</button></div>{rescan && <p role="status">Rescanning unmatched media — {rescan.complete} / {rescan.total}{rescan.finished ? " complete" : ""}</p>}<div className="table-scroll"><table className="data-table"><thead><tr><th></th><th>Filename</th><th>Size / received</th><th>Source / status</th><th>Suggested presenter</th><th>Suggested session</th><th>Suggested presentation</th><th>Confidence / evidence</th><th>Actions</th></tr></thead><tbody>{pageRows.map((item) => { const candidate=item.suggested_candidate; const id=item.media_import_id; const evidence=item.match_candidates[0]?.evidence ?? []; return <><tr key={id}><td><input type="checkbox" aria-label={`Select ${item.original_filename}`} checked={checked.has(id)} disabled={!isGood(item)} onChange={() => setChecked((current) => { const next=new Set(current); if(next.has(id)) next.delete(id); else next.add(id); return next; })}/></td><td><strong>{item.original_filename}</strong></td><td>{formatBytes(item.size_bytes)}<small className="block">{formatDate(item.created_at)}</small></td><td>{item.origin}<small className="block"><MediaStatusBadge value={item.match_state}/></small></td><td>{candidate?.presenters.map((presenter) => presenter.display_name).join(", ") || "—"}</td><td>{candidate?.session_title || "—"}<small className="block">{candidate?.room || "—"} · {formatDate(candidate?.starts_at)}</small></td><td>{candidate ? `${candidate.presentation_identifier} — ${candidate.title}` : "—"}</td><td>{item.match_candidates[0]?.confidence || "none"}<small className="block">{evidence[0] || item.match_reason || "No evidence"}</small></td><td><button className="button button--small" onClick={() => setExpanded(expanded === id ? undefined : id)}>{expanded === id ? "Hide Details" : "Details / Match"}</button><button className="button button--small" onClick={() => void rescanOne(item)}>Rescan</button><button className="button button--small button--primary" disabled={!isGood(item)} onClick={() => void confirmOne(item)}>Confirm</button>{errors.get(id) && <small className="error-text block" role="alert">{errors.get(id)}</small>}</td></tr>{expanded === id && <tr key={`${id}-detail`}><td colSpan={9}><MatchControl item={item} candidates={candidate ? [candidate] : []} eventId={eventId} onDone={() => markConfirmed([id])} onRescanned={(updated) => setItems((current) => mergeMediaRows(current, [updated]))} onSelectionChange={() => undefined} csrf={csrf} /></td></tr>}</>; })}</tbody></table></div>{!visible.length && <Empty title="No media matches this filter" />}<div className="pagination"><button className="button" disabled={!page} onClick={() => setPage(page-1)}>Previous</button><span>{visible.length ? page*REVIEW_PAGE_SIZE+1 : 0}–{Math.min((page+1)*REVIEW_PAGE_SIZE,visible.length)} of {visible.length}</span><button className="button" disabled={(page+1)*REVIEW_PAGE_SIZE>=visible.length} onClick={() => setPage(page+1)}>Next</button></div></Panel>;
}
...
function reviewFilter(item: CentralMediaImport, filter: string) { if(filter==="all") return true; if(filter==="confirmed") return item.match_state==="confirmed"; if(filter==="suggested") return item.match_state==="suggested"; if(filter==="unmatched") return ["unmatched","ambiguous"].includes(item.match_state); if(filter==="errors") return item.import_state==="failed" || Boolean(item.error_detail); return item.match_state!=="confirmed" && item.import_state!=="failed"; }
...
export function isGood(item: CentralMediaImport) { return item.match_state === "suggested" && item.suggested_candidate?.presentation_id === item.match_candidates[0]?.presentation_id && item.match_candidates[0].confidence === "high" && (item.match_candidates.length === 1 || item.match_candidates[0].score > item.match_candidates[1].score); }
...
export function selectedConfirmations(items: CentralMediaImport[], selected: Set<string>, overrides = new Map<string, string>()) {
  return items.filter((item) => selected.has(item.media_import_id)).map((item) => ({ media_import_id: item.media_import_id, presentation_id: overrides.get(item.media_import_id) || selectedCandidate(item) })).filter((item) => Boolean(item.presentation_id));
}
