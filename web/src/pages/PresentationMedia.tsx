...
export function PresentationMedia({ mode }: { mode: Mode }) {
  ...
  const [confirmedIntake, setConfirmedIntake] = useState<Set<string>>(new Set());
  const [candidateOverrides, setCandidateOverrides] = useState<Map<string, string>>(new Map());
  ...
  const data = useApi(async (signal) => {
    ...
    return { ...centralRows(workspace.imports, presentations), summary: workspace.summary, imports: workspace.imports, candidates: [], siteId: "", unmatched: workspace.imports.filter((item) => !confirmedIntake.has(item.media_import_id) && item.match_state !== "confirmed") };
  }, [mode, activeEvent, session?.csrfToken, confirmedIntake]);
  ...
  return <Page eyebrow={mode === "central" ? "Global media control" : "Site-local media"} title="Presentation Media" description={mode === "central" ? "Upload, match, version, and track presentation media across Sites." : "Local uploads remain fully available while Central or the WAN is unavailable."} actions={<button className="button button--primary" disabled={!activeEvent} onClick={() => setUpload(true)}>Upload media</button>}>
    ...
    {data.loading ? <Loading /> : data.error ? <ErrorSurface error={data.error} onRetry={data.refresh} /> : data.data ? <>
      ...
      <Panel title="Match Session / Presenter" description="Suggestions remain unassigned until you explicitly confirm them.">{data.data.unmatched.length ? <>{mode === "central" && <div className="media-toolbar media-toolbar--review"><label>Search review<input className="input" value={reviewSearch} onChange={(event) => setReviewSearch(event.target.value)} placeholder="Filename, presenter, or ID" /></label><button className="button" onClick={() => setChecked(new Set(goodMatchIds(data.data!.unmatched as CentralMediaImport[], reviewSearch, filter)))}>Select All Good Matches</button><button className="button" onClick={() => setChecked(new Set(data.data!.unmatched.filter((item) => reviewVisible(item as CentralMediaImport, reviewSearch, filter) && selectedCandidate(item as CentralMediaImport)).map((item) => String(item.media_import_id))))}>Select All Visible</button><button className="button" onClick={() => setChecked(new Set())}>Clear Selection</button><button className="button button--primary" disabled={!checked.size} onClick={() => { const items = selectedConfirmations(data.data!.unmatched as CentralMediaImport[], checked, candidateOverrides); void centralApi(session.csrfToken).confirmMedia(items).then((result) => { setConfirmedIntake((current) => new Set([...current, ...result.results.filter((item) => item.status !== "failed").map((item) => item.media_import_id)])); setChecked(new Set()); }); }}>Confirm Selected ({checked.size})</button></div>}<div className="unmatched-grid">{data.data.unmatched.filter((item) => reviewVisible(item as CentralMediaImport, reviewSearch, filter)).map((item) => <article key={String(item.media_import_id || item.media_object_id)}>{mode === "central" && <input type="checkbox" aria-label={`Select ${item.original_filename}`} checked={checked.has(String(item.media_import_id))} disabled={!candidateOverrides.get(String(item.media_import_id)) && !selectedCandidate(item as CentralMediaImport)} onChange={() => setChecked((current) => { const next = new Set(current); if (next.has(String(item.media_import_id))) next.delete(String(item.media_import_id)); else next.add(String(item.media_import_id)); return next; })} />}<strong>{String(item.original_filename || item.filename || "Uploaded media")}</strong><small>{String(item.source_relative_path || "Individual file upload")} · {formatBytes(Number(item.size_bytes) || undefined)}</small><MediaStatusBadge value={item.match_state || item.availability || "unassigned"} />{mode === "central" && <MatchControl item={item as CentralMediaImport} candidates={data.data!.candidates || []} eventId={activeEvent} onSelectionChange={(presentationId) => setCandidateOverrides((current) => new Map(current).set(String(item.media_import_id), presentationId))} onDone={() => setConfirmedIntake((current) => new Set(current).add(String(item.media_import_id)))} csrf={session.csrfToken} />}</article>)}</div></> : <Empty title="No media needs review" />}</Panel>
...
export function MatchControl({ item, candidates, eventId, onDone, onSelectionChange, csrf }: { item: CentralMediaImport; candidates: PresentationMatchCandidate[]; eventId: string; onDone: () => void; onSelectionChange?: (presentationId: string) => void; csrf: string | null }) {
