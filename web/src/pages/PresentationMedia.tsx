...
export type MediaSortKey = "filename" | "size" | "received" | "source" | "status" | "presenter" | "session" | "presentation" | "confidence";
...
export function CentralReviewQueue({ initialItems, eventId, csrf }: { initialItems: CentralMediaImport[]; eventId: string; csrf: string | null }) {
  ...
  const visible = useMemo(() => items.filter((item) => reviewFilter(item, filter) && reviewVisible(item, search)).sort((left, right) => compareMediaRows(left, right, sort)), [filter, items, search, sort]);
  const pageRows = visible.slice(page * REVIEW_PAGE_SIZE, (page + 1) * REVIEW_PAGE_SIZE);
  const markConfirmationAccepted = (ids: string[], state = "confirmation_pending") => { setItems((current) => current.map((item) => ids.includes(item.media_import_id) ? { ...item, match_state: state, import_state: "assigned" } : item)); setChecked((current) => new Set([...current].filter((id) => !ids.includes(id)))); };
