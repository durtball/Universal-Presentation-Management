...
export function SitePresentationMedia() {
  ...
  useEffect(() => setLocalRows(intake.data?.items ?? []), [intake.data?.items]);
  const rows = useMemo(() => localRows.filter((row) => siteReviewFilter(row, filter)), [filter, localRows]);
  const rescanPage = async () => { setRescanning(true); try { const updated = await siteApi.mediaIntake(activeEvent, { search: query || undefined, limit: PAGE_SIZE, offset }); setLocalRows(updated.items); } finally { setRescanning(false); } };
...
export function siteReviewFilter(row: Row, filter: string) { if (filter === "all") return true; if (filter === "needs_review") return row.match_state !== "confirmed" && !row.failure_reason; if (filter === "suggested") return row.match_state === "suggested"; if (filter === "confirmed") return row.match_state === "confirmed"; if (filter === "unmatched") return ["unmatched", "ambiguous", "needs_review"].includes(String(row.match_state)); return Boolean(row.failure_reason); }
