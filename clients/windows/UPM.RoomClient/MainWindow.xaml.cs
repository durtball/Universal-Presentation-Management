...
public sealed partial class MainWindow : Window
{
  ...
  private async Task RefreshAsync()
  {
    ...
    catch
    {
      ...
      OfflineBar.Title = "ROOM AGENT UNAVAILABLE"; OfflineBar.Message = "Start UPM.RoomAgent.Service. The Room Client will reconnect automatically.";
    }
  }
  ...
  private void Render(AgentDashboard state)
  {
    ...
    SiteStatusText.Text = state.SiteConnected ? "CONNECTED" : "OFFLINE"; OfflineBar.IsOpen = !state.SiteConnected; OfflineBar.Title = "SITE OFFLINE"; OfflineBar.Message = "Using cached data. Presentations already downloaded remain available.";
  ...
  private async void RestartServiceClick(object sender, RoutedEventArgs e) { try { Process.Start(new ProcessStartInfo("powershell.exe", "-NoProfile -Command \"Restart-Service 'UPM Room Agent'\"") { UseShellExecute = true, Verb = "runas" }); } catch (Exception ex) { await Message("Restart unavailable", ex.Message); } }
